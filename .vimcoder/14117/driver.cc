...
vector <int>	redY;
		...
		__in >> __expected >> blueX >> blueY >> redX >> redY;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << blueX << ", " << blueY << ", " << redX << ", " << redY << std::endl;
		std::cout.flush();

		__timer_start();

		RedAndBluePoints __object;
		int __actual = __object.find(blueX, blueY, redX, redY);
