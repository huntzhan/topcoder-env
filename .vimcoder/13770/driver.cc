...
#include "RailroadSwitchOperator.cc"
...
vector <int>	t;
		...
		__in >> __expected >> N >> x >> t;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << x << ", " << t << std::endl;
		std::cout.flush();

		__timer_start();

		RailroadSwitchOperator __object;
		int __actual = __object.minEnergy(N, x, t);
