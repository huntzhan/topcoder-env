...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> donations;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << donations << std::endl;
		std::cout.flush();

		__timer_start();

		BadNeighbors __object;
		int __actual = __object.maxDonations(donations);
