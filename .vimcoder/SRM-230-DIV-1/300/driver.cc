...
int	time;
		...
		__in >> __expected >> c >> time;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << c << ", " << time << std::endl;
		std::cout.flush();

		__timer_start();

		SortEstimate __object;
		double __actual = __object.howMany(c, time);
