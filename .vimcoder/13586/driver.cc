...
vector <int>	lastWeek;
		...
		__in >> __expected >> firstWeek >> lastWeek;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << firstWeek << ", " << lastWeek << std::endl;
		std::cout.flush();

		__timer_start();

		AttackOfTheClones __object;
		int __actual = __object.count(firstWeek, lastWeek);
