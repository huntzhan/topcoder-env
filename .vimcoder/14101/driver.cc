...
int	newB;
		...
		__in >> __expected >> a >> b >> newA >> newB;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << a << ", " << b << ", " << newA << ", " << newB << std::endl;
		std::cout.flush();

		__timer_start();

		DoubleOrOneEasy __object;
		int __actual = __object.minimalSteps(a, b, newA, newB);
