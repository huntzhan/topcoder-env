...
int	numMoves;
		...
		__in >> __expected >> size >> start >> end >> numMoves;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << size << ", " << start << ", " << end << ", " << numMoves << std::endl;
		std::cout.flush();

		__timer_start();

		ChessMetric __object;
		long long __actual = __object.howMany(size, start, end, numMoves);
