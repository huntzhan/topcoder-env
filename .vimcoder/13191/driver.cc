...
int	D;
		...
		__in >> __expected >> steps >> T >> D;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << steps << ", " << T << ", " << D << std::endl;
		std::cout.flush();

		__timer_start();

		RevengeOfTheSith __object;
		int __actual = __object.move(steps, T, D);
