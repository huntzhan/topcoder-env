...
vector <string>	ciphertexts;
		...
		__in >> __expected >> plaintexts >> ciphertexts;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << plaintexts << ", " << ciphertexts << std::endl;
		std::cout.flush();

		__timer_start();

		DecodeMoveToFront __object;
		string __actual = __object.findKey(plaintexts, ciphertexts);
