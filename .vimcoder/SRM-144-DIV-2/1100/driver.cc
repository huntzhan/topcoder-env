...
vector <int>	ductLength;
		...
		__in >> __expected >> fromJunction >> toJunction >> ductLength;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << fromJunction << ", " << toJunction << ", " << ductLength << std::endl;
		std::cout.flush();

		__timer_start();

		PowerOutage __object;
		int __actual = __object.estimateTimeOut(fromJunction, toJunction, ductLength);
