...
string	isGood;
		...
		__in >> __expected >> n >> m >> x >> a >> b >> c >> isGood;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << m << ", " << x << ", " << a << ", " << b << ", " << c << ", " << isGood << std::endl;
		std::cout.flush();

		__timer_start();

		BagAndCards __object;
		int __actual = __object.getHash(n, m, x, a, b, c, isGood);
