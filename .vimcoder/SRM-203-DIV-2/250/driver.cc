...
string	newName;
		...
		__in >> __expected >> existingNames >> newName;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << existingNames << ", " << newName << std::endl;
		std::cout.flush();

		__timer_start();

		UserName __object;
		string __actual = __object.newMember(existingNames, newName);
