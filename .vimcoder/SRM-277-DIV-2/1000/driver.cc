...
int	n;
		...
		__in >> __expected >> lowerBound >> upperBound >> n;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << lowerBound << ", " << upperBound << ", " << n << std::endl;
		std::cout.flush();

		__timer_start();

		UnionOfIntervals __object;
		int __actual = __object.nthElement(lowerBound, upperBound, n);
