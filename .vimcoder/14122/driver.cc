...
vector <int>	productivity;
		...
		__in >> __expected >> manager >> salary >> productivity;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << manager << ", " << salary << ", " << productivity << std::endl;
		std::cout.flush();

		__timer_start();

		FiringEmployees __object;
		int __actual = __object.fire(manager, salary, productivity);
