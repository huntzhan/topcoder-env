...
int	term;
		...
		__in >> __expected >> loan >> interest >> term;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << loan << ", " << interest << ", " << term << std::endl;
		std::cout.flush();

		__timer_start();

		Mortgage __object;
		int __actual = __object.monthlyPayment(loan, interest, term);
