...
int	loanTerm;
		...
		__in >> __expected >> price >> monthlyPayment >> loanTerm;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << price << ", " << monthlyPayment << ", " << loanTerm << std::endl;
		std::cout.flush();

		__timer_start();

		AutoLoan __object;
		double __actual = __object.interestRate(price, monthlyPayment, loanTerm);
