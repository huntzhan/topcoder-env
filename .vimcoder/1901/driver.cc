...
int	workers;
		...
		__in >> __expected >> folders >> workers;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << folders << ", " << workers << std::endl;
		std::cout.flush();

		__timer_start();

		FairWorkload __object;
		int __actual = __object.getMostWork(folders, workers);
