...
vector <int>	droids;
		...
		__in >> __expected >> doors >> droids;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << doors << ", " << droids << std::endl;
		std::cout.flush();

		__timer_start();

		ThePhantomMenace __object;
		int __actual = __object.find(doors, droids);
