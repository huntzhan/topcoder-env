...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> stripes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << stripes << std::endl;
		std::cout.flush();

		__timer_start();

		StripePainter __object;
		int __actual = __object.minStrokes(stripes);
